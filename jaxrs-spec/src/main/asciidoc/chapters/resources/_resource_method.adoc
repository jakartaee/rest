...
* For resource methods that consume `multipart/form-data` media types,
the `@FormParam` annotation may be used on `jakarta.ws.rs.core.EntityPart`
parameters. See <<consuming_multipart_formdata>> for more details.
